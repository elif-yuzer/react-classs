import uuid from "react-uuid";

const initial = {
  doctorlar: [
    {
      id: 1,
      doctorName: "DR Ahmet Bilen",
      doctorImg:
        "https://i.cnnturk.com/i/cnnturk/75/0x555/54857220f97adb1aa472e5e6",
    },
    {
      id: 2,
      doctorName: "DR Ayse Okur",
      doctorImg:
        "https://i3.posta.com.tr/i/posta/75/750x0/616f757845d2a0b25401f0e1",
    },
    {
      id: 3,
      doctorName: "DR Fatma Adil",
      doctorImg:
        "https://i4.hurimg.com/i/hurriyet/75/0x0/5efd779645d2a04258b8f1cc.jpg",
    },
    {
      id: 4,
      doctorName: "DR Oya Başar",
      doctorImg:
        "https://im.haberturk.com/2019/09/12/2521591_720f96f71c734286b9c93122b8bbd70c_640x640.jpg",
    },
  ],
  hastalar: [
    {
      id: 1,
      patientName: "Yavuz Selim",
      day: "Dec 12th at 2:30pm",
      isDone: false,
      myDoctor: "DR Ayse Okur",
    },
    {
      id: 2,
      patientName: "Mihrimah Sultan",
      day: "Dec 13th at 1:30pm",
      isDone: true,
      myDoctor: "DR Fatma Adil",
    },
    {
      id: 3,
      patientName: "Fatih Mehmet",
      day: "Dec 14th at 3:00pm",
      isDone: false,
      myDoctor: "DR Oya Başar",
    },
    {
      id: 4,
      patientName: "İpek Bilir",
      day: "Dec 12th at 2:30pm",
      isDone: false,
      myDoctor: "DR Ahmet Bilen",
    },
    {
      id: 5,
      patientName: "Haluk Yilmaz",
      day: "Dec 13th at 1:30pm",
      isDone: true,
      myDoctor: "DR Ahmet Bilen",
    },
    {
      id: 6,
      patientName: "Ayşe Güler",
      day: "Dec 14th at 3:00pm",
      isDone: false,
      myDoctor: "DR Oya Başar",
    },
  ],

  tiklayinca: true,
};


export const appReducer= (state=initial,action)=>{
    const {type,payload}=action
    switch (type) {
        case "DOCTORCLICK" :
            return {
                ...state,
                doctorlar:state.doctorlar.filter((doctor)=>doctor.id===payload)
            }
        case "HASTASIL" :
            return {
                ...state,
                hastalar: state.hastalar.filter((hasta)=>hasta.id!==payload)
            }
        case "HASTACLICK" :
            return {
                ...state,
                hastalar: state.hastalar.filter((hasta)=>hasta.id==payload)
            }
        case "CHANGE" :
            return {
                ...state,
                hastalar:state.hastalar.map((hasta)=>hasta.id===payload ? {...hasta, isDone:!hasta.isDone} : hasta)
            }
        case "TIKLA" :
            return {
              
                tiklayinca:false
                
            }
        case "POST":
         return {
         ...state,
        hastalar: [
          ...state.hastalar,
        {
        id: uuid(),
        patientName: payload.hastaAdi,
        day: payload.tarih,
        isDone: false,
        myDoctor: state.doctorlar[0].doctorName,
      },
    ],
  };
        
        
    
        default:
            return state
    }

}

